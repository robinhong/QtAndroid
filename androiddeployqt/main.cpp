...
QString packageNameFromGradleBuildFile(const QString &path)
{
    QFile file(path);
    ...
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        for(const auto &line : lines)
        {
            ...
            {
                return QLatin1String(line.trimmed().replace("namespace", "").replace('\'', "").replace('\"', ""));
            }
            else if(line.contains("applicationId"))
            {
                return QLatin1String(line.trimmed().replace("applicationId", "").replace('\'', "").replace('\"', ""));
            }
        }
    }
    return {};
...
bool readInputFile(Options *options)
{
    ...
    {
        options->packageName = packageNameFromGradleBuildFile(options->outputDirectory + QLatin1String("build.gradle"));
    }
    if(options->packageName.isEmpty())
    {
...
GradleFlags gradleBuildFlags(const QString &path)
{
    ...
    for(const auto &line : lines)
    {
        ...
        {
            fprintf(stdout, line.trimmed().replace("namespace", "").replace('\'', "").replace('\"', ""));
            fprintf(stdout, "\n");
        }
    }
